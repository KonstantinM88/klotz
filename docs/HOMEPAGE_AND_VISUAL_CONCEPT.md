...
## Редизайн v2 — актуальное направление после обратной связи

Предыдущая реализация признана пользователем слишком близкой к legacy. Ниже сохранена исходная концепция для истории; её split-hero, монотонная карточная сетка и прежняя teal-палитра заменены решением D-018.

- Первый экран: реальная full-bleed фотография Glasschiebewände, H1 `Raum für mehr Leben.`, позиционирование `Bauelemente & Outdoor Living` и Merseburg остаются видимыми.
- Уточнение 25.09: под эмоциональным H1 появилось краткое перечисление основных услуг и регионального контекста. Перед финальным CTA добавлен редакционный FAQ из пяти вопросов; это продолжение пути выбора, а не новый визуальный стиль. Ответы и ссылки доступны в HTML без клиентского JavaScript.
- Типографика: Manrope + системная Georgia italic для акцентных строк. Дополнительные сетевые шрифты не добавляются.
- Материалы: графит `#202822`, тёплая бумага `#f3f2eb`, минеральный фон, акцент `#d9ed97`. Оригинальный синий логотип сохранён, не перерисован.
- Разные композиции: редакционное вступление; 4 асимметричных service entries; крупная фотосерия референсов; sticky-вступление и вертикальные шаги; фотоблок региона; B2B; журнал; типографический CTA; крупная подпись в подвале.
- Motion: однократное прорисовывание координатных линий в hero; мягкое появление секций; image-hover и реакции стрелок. Никаких видео, WebGL, перехвата прокрутки или новых motion-пакетов. Reduced motion и отсутствие JS не скрывают контент.
- Уточнение 25.09: плавающий запуск `Projektlotse` на главной стал компактным icon-only элементом с полупрозрачной поверхностью, задержкой 5,5 секунды и коротким CSS-импульсом. Это не автоматическое открытие диалога; при reduced motion элемент появляется без движения.
- Общий язык распространяется на внутренние маршруты: масштаб заголовков, материалы, границы, кнопки, карточки, статьи, форма, footer.
- Цветовой акцент является предложением для обсуждения, не объявляется новым утверждённым brand book.

## Архив исходной концепции v1
